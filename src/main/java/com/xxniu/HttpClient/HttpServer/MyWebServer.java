package com.xxniu.HttpClient.HttpServer;
...
//			Map<String, Object> objRes = JsonUtils.of().toObject(msg.toString(), Map.class);
//			System.out.println("cardType" + objRes.get("cardType"));
			System.out.println("接收到客户端的请求:" + msg);
			// 下面是由服务器直接生成的主页内容
			// 1、首先向浏览器输出响应头信息
...
//					+ "</center>您提交的数据如下:<pre>" + msg.toString() + "</pre></BODY></HTML>");
			out.println("{\"status\":0,\"msg\":\"SUCCESS\"}");
			out.flush();
			out.close();
			is.close();
